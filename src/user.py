...
class User:
    ...
    def question(self):
        """
        Get current question raw mwssage text.
        """
    ...
    def current_question(self):
        ...
        """
        question_text = ':pencil: <strong>Qestion Preview</strong>\n\n'
        question_text += self.question
        question_text += f'\n{"_" * 40}\nWhen done, click <strong>{keys.send_question}</strong>.'
        return question_text

    def save_qestion(self):
        """
        ...
        self.db.users.questions.insert_one({
    ...
    def reset(self):
        ...
        def send_question_to_all(self):
            ...
            with concurrent.futures.ThreadPooExecutor() as executor:
                for chat_id in self.db.users.distinct('chat.id'):
                    executor.submit(
                        self.stackbot.send_message, chat_id, msg_text)

            self.send_message(
                text=':check_mark_button: Question sent successfully to all users.')


if __name__ == '__main__':
    u = User(chat_id=273365147)
    print(u.state)
